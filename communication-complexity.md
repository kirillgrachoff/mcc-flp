...
Коммуникационной сложностью функции $f$ - $D(f)$ называется минимальное количество бит коммуникации, которого достаточно для решения поставленной задачи в худшем случае (то есть этого количества битов должно быть достаточно для любой пары $x,y$).

## Определение (`Multi-Communication Complexity`, `MCC`)
Это "generalized communication complexity" или "number in hand model".
Отличиями является то, что
- $n \ge 3$.
- функцию должны вычислить все узлы.

## Определение (`Multipart Communication Complexity`)
Важно отличать эту задачу от предыдущей.
- Пусть есть *узлы* $P_1 \dots P_n$, где каждый $P_i$ знает $x_j \forall j \neq i$.
- Требуется вычислить $f(x_1, \dots x_n)$ для каждого участника.
- Коммуникация происходит через меловую доску, где каждый участник может видеть все сообщения.

## Определение (`Channel`)
- Сообщение $m$ доставляется от отправителя получателю.
- Сообщение $m$ может дойти только полностью и не изменённым.

## Определение (`Half Duplex Channel`)
- Можно либо принимать сообщение, либо посылать сообщение.
- Если оба игрока отсылают сообщение, то ни одно из них не доставляется.
- Если оба игрока принимают сообщение, то в зависимости от модели может произойти следующее:
  - Принимается специальный символ *nop*
  - Принимается $0$
  - Принимается *шум* - случайный бит

## Определение (`Communication Problem with Half Duplex Channel`)
- Задача `CC` с использованием `Half Duplex Channel`.
  
Оценки на верхнюю и нижнюю границы различных задач могут отличаться в зависимости от того, какое поведение half-duplex channel использовано.

## Определение (`Asynchronous channel`)
Это канал передачи сообщений со следующими условиями:
- Время доставки сообщения может быть сколь угодно большим, но конечным.
